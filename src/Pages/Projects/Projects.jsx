    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8 my-12">
        {projects?.map((project, i) => (
          ...
          >
            {/* Image */}
            <div data-aos="fade-up" data-aos-delay={i * 100} className="overflow-hidden group">
              <img
                src={project.image}
                loading="lazy"
                alt={project.name}
      ...
      {selectedProject && (
        <div className="fixed top-0 left-0 h-screen w-screen bg-black/40 backdrop-blur-xl flex items-center justify-center z-50">
          <div className="relative bg-[#1a1a1a] rounded-xl w-full max-w-3xl overflow-y-auto max-h-[90vh]">
            <button
              onClick={() => setSelectedProject(null)}
              className="absolute top-4 right-4 px-3 py-2 bg-red-500 text-white rounded-xl"
            >
              X
            </button>
            <ViewDetails
